...
def _clean_user(user):
    if not app_settings.STAR_RATINGS_ANONYMOUS:
        if not user:
            raise ValueError(
                _("User is mandatory. Enable 'STAR_RATINGS_ANONYMOUS' for anonymous ratings."))
        return user
    return None
...
class RatingManager(models.Manager):
    ...
    def rate(self, instance, score, user=None, ip=None):
        ...
        existing_rating = UserRating.objects.for_instance_by_user(
            instance, user)
        ...
        if existing_rating:
            ...
            rating, created = self.get_or_create(
                content_type=ct, object_id=instance.pk)
...
class UserRatingManager(models.Manager):
    ...
    def bulk_create(self, objs, batch_size=None):
        objs = super(UserRatingManager, self).bulk_create(
...
class UserRating(TimeStampedModel):
    ...
    rating = models.ForeignKey(get_star_ratings_rating_model_name(
